...
for (iter in 1:(nb+ns)){
  ...
  tmp = mclapply(1:nABC,FUN = function(x){
    return(mcars_c(y_listABC[[x]], av[,hashABC==x], mu+muABC[,x], Rinfo))
  },mc.cores = ncores)
  # update random effects muABC
  for (j in 1:nABC){
    av[,hashABC==j] = tmp[[j]]
    dmx = mMmu_M(mu, tmp[[j]], FALSE)
    tmpS = solve(SigmaABC_inv + ncol(dmx)*Sigma_inv)
    muABC[,j] = rmvnorm(1,tmpS%*%apply(Sigma_inv%*%dmx,1,sum),tmpS)
  }
  # update SigmaABC
  ...
  if (iter <= nb/2){
    ...
    for (j in 1:nABC){
      av[,hashABC==j] = mMmu_M(muABC[,j], av[,hashABC==j], FALSE)
    }
    mu = apply(av,1,mean)
    Sigma = cov(t(av))
    vmu = cbind(vmu,mu)
    ...
    for (j in 1:nABC){
      av[,hashABC==j] = mMmu_M(muABC[,j], av[,hashABC==j], TRUE)
    }
  }
}
